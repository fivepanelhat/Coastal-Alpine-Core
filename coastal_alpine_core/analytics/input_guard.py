...
def input_guard_check(device_id, payload):
    ...
    try:
        ...
        if not expected_hash or posture.get("firmware_hash") != expected_hash:
            print(f"[SECOPS ANOMALY] Posture validation failed for {device_id}! Rogue firmware detected.")
        ...
        if len(history) > 10:
            ...
            if std_dev > 0.5:
                ...
                if z_score > 3.5:
                    print(f"[ALERT] Statistical anomaly flagged on {device_id}. Value: {current_val} (Z-Score: {z_score:.2f})")
